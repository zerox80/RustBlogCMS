...
async fn test_bug_fixes() -> anyhow::Result<()> {
    ...
    sqlx::query(
        r#"CREATE TABLE tutorials (
            id TEXT PRIMARY KEY, title TEXT, description TEXT, icon TEXT,
            color TEXT, topics TEXT, content TEXT, version INTEGER,
    ...
    sqlx::query(
        r#"INSERT INTO tutorials (
            id, title, description, icon, color, topics, content,
    ...
    let tutorials = sqlx::query_as::<_, Tutorial>(
        r#"SELECT id, title, description, icon, color, topics, '' as content,
                  version, created_at, updated_at
           FROM tutorials"#,
    )
    .fetch_all(&pool)
