...
mod tests {
    ...
    fn test_secret_entropy_validation() {
        ...
        assert!(!secret_has_min_entropy("ThisIsAVeryLongSecretWithUppercaseAndLowercaseButNoNumbersOrSpecialChars"));
        
        // Too few unique characters (fails uniqueness check)
        assert!(!secret_has_min_entropy("A1!A1!A1!A1!A1!A1!A1!A1!A1!A1!A1!A1!A1!A1!A1!A1!A1!A1!A1!A1!A1!A1!"));
        
        // Valid high entropy secret (meets all requirements)
        assert!(secret_has_min_entropy("p@ssW0rd_Extremely_Long_And_Secure_With_Many_Chars_123!"));
    }
    ...
    fn test_jwt_initialization_flow() {
        ...
        env::set_var("JWT_SECRET", "this_is_a_test_secret_with_adequate_entropy_123_ABC_!!!");
        let result = init_jwt_secret();
        
        match result {
            Ok(_) => assert!(JWT_SECRET.get().is_some()),
            Err(e) => assert!(e.contains("already initialized") || e.contains("JWT_SECRET already initialized")),
        }
    }
    ...
    fn test_jwt_create_and_verify() {
        ...
        if JWT_SECRET.get().is_none() {
            env::set_var("JWT_SECRET", "this_is_another_test_secret_with_adequate_entropy_123_XYZ_!!!");
    ...
    fn test_parse_bearer_token() {
        assert_eq!(parse_bearer_token("Bearer my_token"), Some("my_token".to_string()));
        assert_eq!(parse_bearer_token("bearer  my_token "), Some("my_token".to_string()));
