...
const validateTutorialId = (tutorialId) => {
  ...
  return tutorialId.length > 0 && tutorialId.length <= 100 && /^[a-zA-Z0-9_-]+$/.test(tutorialId)
}
...
export const markAsRead = (tutorialId, options = {}) => {
  ...
  try {
    ...
    progress[tutorialId] = {
      ...
      completedSections: Array.isArray(options.completedSections) ? options.completedSections : [],
      version: '2.0.0',
    }
...
export const getProgressStatistics = () => {
  try {
    ...
    const completionRate =
      tutorialIds.length > 0 ? Math.round((completedIds.length / tutorialIds.length) * 100) : 0
