...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (cooldownUntil && now < cooldownUntil) {
      ...
      setError(
        `Zu viele Anmeldeversuche. Bitte warte ${remainingSeconds} Sekunde${remainingSeconds === 1 ? '' : 'n'}.`,
      )
      return
    }
    ...
    if (!/^[a-zA-Z0-9_.-]{1,50}$/.test(trimmedUsername)) {
      setError(
        'Benutzername darf nur Buchstaben, Zahlen sowie _ . - enthalten und max. 50 Zeichen lang sein.',
      )
      return
    }
...
bg-primary-600/20 blur-[120px] animate-float`}
        />
        <div
          className={`absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full
bg-accent-violet/20 blur-[120px] animate-float`}
          style={{ animationDelay: '-3s' }}
        />
        <div
          className={`absolute top-[20%] right-[20%] w-[20%] h-[20%] rounded-full
bg-accent-cyan/20 blur-[80px] animate-pulse-slow`}
        />
      </div>
      ...
      <div className="relative w-full max-w-md p-4 animate-scale-in">
        <div
          className={`relative bg-surface-900/60 backdrop-blur-xl border border-white/10
rounded-3xl shadow-card-xl overflow-hidden`}
        >
          {/* Decorative top line */}
          <div
            className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent
via-primary-500 to-transparent opacity-50`}
          />
...
from-primary-500/20 to-accent-violet/20 rounded-2xl mb-6 ring-1
ring-white/10 shadow-inner-glow animate-float`}
              >
            ...
            {error && (
              <div
                className={`mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-xl flex items-start
space-x-3 animate-slide-down`}
              >
...
transition-colors group-focus-within:text-primary-400 text-surface-400`}
                  >
                  ...
                  <input
                    ...
                    className={`block w-full pl-11 pr-4 py-3.5 bg-surface-800/50 border border-surface-700
rounded-xl focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500
text-white placeholder-surface-500 transition-all duration-200 outline-none
...
border-transparent rounded-xl text-white font-semibold bg-gradient-to-r
from-primary-600 to-accent-violet hover:from-primary-500
hover:to-accent-violet/90 focus:outline-none focus:ring-2
focus:ring-offset-2 focus:ring-primary-500 focus:ring-offset-surface-900
...
duration-200 flex items-center justify-center gap-2 mx-auto group`}
          >
